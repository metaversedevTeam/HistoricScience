...
{
    // 세그먼트 양 끝점에 맞춰 위치와 회전을 잡고 길이를 적용한다. 벽 형태의 오브젝트/이펙트로 교체해도
    // 자연스럽도록 회전은 Y축(요)만 조절해 항상 수직으로 세우고, 높이는 두 끝점 중 낮은 쪽에 맞춘다.
    public void SetSegment(Vector3 start, Vector3 end)
    {
        ...
        HandleApplyLength(direction.magnitude);
    }
    ...
    // 세그먼트 길이에 맞춰 이펙트의 크기/방출량을 조정한다
    protected abstract void HandleApplyLength(float length);
}
