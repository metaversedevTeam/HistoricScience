...
// 청크 터레인이 구워진 뒤, 각 아이템 데이터에 연결된 소환 방식이 계산한 자리에 자원 소스 프리팹을 소환하는 컴포넌트
public class ChunkResourceSpawner : MonoBehaviour
{
    ...
    public void SpawnResources(Vector2 mapViewOrigin, float mapViewSize, MapData mapData)
    {
        ...
        {
            if (item == null || item.SourcePrefab == null || item.SpawnRule == null)
                continue;

            HandleSpawnItem(item, context, container);
        }
    }

    // 아이템 하나의 소환 방식에게 소환할 자리를 물어보고, 받은 자리마다 자원 소스를 소환한다.
    private void HandleSpawnItem(ItemData item, in ResourceSpawnContext context, Transform container)
    {
        // 목록을 호출마다 새로 만들어 아이템 사이에 상태를 공유하지 않는다. 나중에 아이템별 계산을 병렬로 돌려도 서로 간섭하지 않는다.
        List<ResourceSpawnPlacement> placements = new List<ResourceSpawnPlacement>();
        item.SpawnRule.GetPlacements(item, context, placements);

        for (int i = 0; i < placements.Count; i++)
            HandleSpawnResource(item.SourcePrefab, container, placements[i]);
    }
