...
{
    // 프리팹에 설정된 기본 이동 속도 대비 지금 이동 속도의 배율. 연구 보너스로 걸음이 빨라지면 1보다 커진다.
    // 이동에 맞춰 애니메이션이나 소리의 빠르기를 조절하는 쪽에서 쓴다.
    float MoveSpeedRatio { get; }
    ...
    // 대기 중이던 콜백이 있는 이동이 새 Move() 명령으로 대체돼 취소됐을 때 발생. 그 콜백들은 호출되지 않고 폐기되므로,
    // 이동 결과를 기다리던 쪽이 정리할 기회를 주기 위한 이벤트다. 이동이 끝나서 발생하는 것이 아니므로 OnMoveEnd와는 별개이며,
    // Stop()으로 취소된 경우는 이동 종료 콜백과 OnMoveEnd로 이미 알려지므로 발생하지 않는다.
    event Action OnMoveOrderReplaced;
