...
// 창고 UI를 여는 창고(Warehouse) 건물 프리팹을 WareHouse 모델로 생성하고, 저장 프리팹 목록과 시민의 건설 목록에 등록하는 에디터 도구
public static class WarehousePrefabTool
{
    public const string PrefabPath = "Assets/Prefabs/Object/Warehouse.prefab";
    private const string ModelPrefabPath = "Assets/Art/Models/Where Hose Model/WareHouse Model.prefab";
    private const string BottomPrefabPath = "Assets/Art/Models/BuildingBottom/Building Bottom.prefab";
    private const string WarehouseUiPrefabPath = "Assets/Prefabs/UI/WarehouseUI.prefab";
    ...
    // 명령 버튼과 건물 아이콘에 쓸 상자 아이콘 이름
    private const string BoxIconName = "Icon_Box";

    // 저장 프리팹 목록과 프리팹의 SavableHandler가 공유해야 하는 식별 키
    private const string PrefabId = "Warehouse";
    // 건설에 필요한 돌 수량
    private const int StoneBuildCost = 8;

    // CapsuleCollider.direction의 Y축 값
    private const int CapsuleDirectionY = 1;

    // 프리팹 안에서 모델·바닥 자식을 다시 찾기 위한 이름. 스케일을 이어받을 때 쓴다.
    private const string ModelChildName = "WareHouse Model";
    ...
    [MenuItem("Tools/HistoricScience/Generate Warehouse Prefab")]
    public static void Generate()
    {
        // 명령 버튼에 쓸 상자 아이콘이 아직 없을 수 있으므로 먼저 굽는다.
        ...
        AddWarehouseComponent(root, model);
        ...
        Debug.Log($"[WarehousePrefabTool] '{PrefabPath}' 프리팹을 생성했습니다. 모델 스케일 {modelScale.x:0.##}, 모델 크기 {modelBounds.size}, 캡슐 반지름 {footprintRadius:0.##}");
    }
    ...
    private static void AddWarehouseComponent(GameObject root, GameObject model)
    {
        ...
        GameObject warehouseUiPrefab = LoadRequired<GameObject>(WarehouseUiPrefabPath);
        Sprite boxIcon = UIProceduralSpriteFactory.LoadIcon(BoxIconName);
        ResourceData stone = LoadRequired<ResourceData>(StoneItemPath);

        if (boxIcon == null)
            Debug.LogWarning($"[WarehousePrefabTool] '{BoxIconName}' 아이콘을 찾지 못해 명령 버튼 아이콘이 비어 있습니다.");
    ...
    // 저장 프리팹 목록에 창고 항목을 추가한다. 같은 PrefabId가 이미 있으면 프리팹 참조만 갱신한다.
    ...
    // 시민의 건물 선택 목록에 창고를 추가해 '건물 짓기'에서 고를 수 있게 한다. 이미 들어 있으면 아무것도 하지 않는다.
    private static void RegisterInCitizenBuildables(GameObject prefabAsset)
    {
        ...
        if (citizen == null)
        {
            Debug.LogWarning($"[WarehousePrefabTool] '{CitizenPrefabPath}'에서 Citizen 컴포넌트를 찾지 못했습니다.");
            return;
        }
    ...
    private static T LoadRequired<T>(string path) where T : Object
    {
        ...
        if (asset == null)
            Debug.LogError($"[WarehousePrefabTool] '{path}' 에셋을 찾지 못했습니다.");
